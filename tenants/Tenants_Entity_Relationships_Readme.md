...
* **Location:** Lives in the `public` schema (metadata) but **owns** a specific PostgreSQL schema (e.g., `demo`) where actual app data resides.
...
* **Relation:** A Domain belongs to **one** Tenant(ForeignKey). It acts as the routing key (e.g., `demo.lvh.me` -> `demo` schema). Multiple domains can point to the same tenant (e.g., `demo.company.com` and `demo.company.net` both route to the `demo` schema).
...
1. **Delete the Domain.**
    * *Why?* If you just want to change `demo.lvh.me` to `app.lvh.me`, you delete the old domain and create a new one linked to the *same* tenant.
